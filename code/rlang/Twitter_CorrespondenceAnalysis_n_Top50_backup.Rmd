...
```{r echo=FALSE}
tweets_eth <- readRDS('tidy_tweets_per_week_n_eth_50.rds')
tweets_facebook <- readRDS('tidy_tweets_per_week_n_facebook_50.rds')
tweets_bitcoin <- readRDS('tidy_tweets_per_week_n_bitcoin_50.rds')
...
#design_col = c('RED', 'BLUE', 'GREEN')

color4Authors <- recode(design, D_Ethurium= design_col[1], D_Facebook=design_col[2], D_Bitcoin = design_col[3])
#prettyGraphsColorSelection(n.colors = nrow(ci.a))
```
...
legend  <- createFactorMap(legend_fi,
                             ...
                             col.labels = design_col,cex = 8,alpha.points = 0.4, alpha.labels = 0.4,text.cex = 8)
...
```{r}
map.IJ.sym <- symMap$baseMap + symMap$I_points + #symMap$I_labels + 
...
```{r echo=FALSE, results='hide'}
color4I <- brewer.pal(n = 12, name='Set3')
color4I <- rep(color4I, length.out = nrow(ci.a))
# baseMaps ----
...
color4J <- brewer.pal(n = 9, name='Set1')
color4J <- rep(color4J, length.out = nrow(cj.a))
