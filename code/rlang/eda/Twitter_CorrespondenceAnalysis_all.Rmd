...
tweets_eth_50 <- readRDS('../../data/tidy_tweets_per_week_n_eth_50.rds')
tweets_facebook_50 <- readRDS('../../data/tidy_tweets_per_week_n_facebook_50.rds')
tweets_bitcoin_50 <- readRDS('../../data/tidy_tweets_per_week_n_bitcoin_50.rds')
...
tweets_eth_50 <- as.data.frame(tweets_eth_50) %>% column_to_rownames(var = 'word')
tweets_facebook_50 <- as.data.frame(tweets_facebook_50) %>% column_to_rownames(var = 'word')
tweets_bitcoin_50 <- as.data.frame(tweets_bitcoin_50) %>% column_to_rownames(var = 'word')
...
tweets_eth_supp <- supplementaryRows(tweets_eth_50,resCA.asym)
asymMap_supp  <- createFactorMapIJ(tweets_eth_supp$fii[tweets_eth_supp$fii[,1] > 0,] ,Fj[Fj[,1] > 0,] ,
                             #col.points.i = color4Authors,
                             #col.labels.i = color4Authors
                             )
...
tweets_eth_50['#geoloc',]

```

```{r}

tweets_eth_50['#freecoin',]
...
tweets_bitcoin_supp <- supplementaryRows(tweets_bitcoin_50,resCA.asym)
asymMap_supp  <- createFactorMapIJ(tweets_bitcoin_supp$fii[tweets_bitcoin_supp$fii[,1] > 0,] ,Fj[Fj[,1] > 0,] ,
                             #col.points.i = color4Authors,
                             #col.labels.i = color4Authors
                             )
...
tweets_bitcoin_50['#freecoin',]

```

```{r}

tweets_bitcoin_50['#crowdfund',]

```

```{r}

tweets_bitcoin_50['#crowdsal',]
